...
import com.app.cocktailapp.core.MockResponse.getDrinksData
...
class DrinksUseCaseTest {
    ...
    fun `Given response data when invoke drinks use case expect result has data`() = runTest {
        coEvery { drinksRepositoryImpl.fetchDrinksByCategory(strCategory) } returns MockResponse.getDrinksData()

        val first = drinksUseCase.invoke(strCategory).drop(1).first()

        Assert.assertEquals(getDrinksData().drinkResponseModels.get(0), first.data?.get(0))
    }

    @Test
    fun `Given http error when invoke drinks use case expect null data`() = runTest {
        //send strCategory empty
